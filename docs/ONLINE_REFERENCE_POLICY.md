...
| Conversational intake tooling | OpenAI Structured Outputs documentation, https://developers.openai.com/api/docs/guides/structured-outputs; OpenAI Function Calling documentation, https://developers.openai.com/api/docs/guides/function-calling | Informed the engineering boundary for the chat-intake workflow: convert unstructured conversation into schema-like fields, then invoke an explicit backend calculation tool. The implemented production path uses a deterministic local parser and existing `/runs/start`; AI/chat never supplies mechanical formulas, APDL authority, or final pass/fail conclusions. Accessed 2026-06-12. |
| Square-tube product standard status | National standards platform, GB/T 6728-2017 detail page, https://std.samr.gov.cn/gb/search/gbDetailed?id=71F772D81812D3A7E05397BE0A0AB82A | Confirms the cold-formed hollow-section product-standard context and that GB/T 6728-2017 was superseded by GB/T 6728-2025 from 2026-03-01. Used only as product/mass context; it does not replace local SECT files or mechanical evaluation rules. Accessed 2026-07-10. |
| Nuclear-project quota management | National Energy Administration, Interim Measures for Nuclear Power Engineering Quota Management, https://www.nea.gov.cn/20250521/11fddeb91fe0424c92a9e8c8fb9cc14a/c.html | Confirms that nuclear-project cost uses approved quotas, bill-of-quantity rules and dynamically collected labor/material/machinery prices. Adopted as the boundary that currency output requires unit-approved nuclear-project pricing data. It does not alter mechanics. Accessed 2026-07-10. |
| Nuclear-island budget quota framework | National Energy Administration, Nuclear Island Engineering Budget Quota (2025 Edition), https://www.nea.gov.cn/20250811/c61925a8e7e141b5801dcdcc4c139d3a/c.html | Confirms the current nuclear-island quota framework includes construction/installation fees, machinery, steel structure, electrical installation and corrosion protection scopes. The public notice does not supply item-level cable-tray-support prices, so no currency value is inferred. Accessed 2026-07-10. |
| Withdrawn ordinary-market price use | Baoji regional construction material reference 02/2026 | The previous ordinary square-tube market snapshot was removed from runtime configuration because it is not a nuclear-project comprehensive cost source. Default ranking now uses deterministic section area/theoretical mass only. |
| Steel price trend provenance | China Iron and Steel Association price-index portal, https://www.chinaisa.org.cn/gxportal/xfgl/portal/index.html | Confirms that steel prices are time-dependent and should be versioned rather than hardcoded as timeless engineering facts. No index value enters current calculations. Accessed 2026-07-10. |
| Login field width | Ant Design ProComponents field width guidance, https://procomponents.ant.design/components/field-set/ | Informed the login form's bounded desktop width. Ant Design classifies 328 px as a standard field and 440 px as suitable for long paths/URLs; CableTrayAI therefore uses 320-360 px for short username/password fields and keeps mobile fields full width. Affects only `apps/web/login.html`. Accessed 2026-07-15. |
| Login input sizing | IBM Carbon text input usage, https://carbondesignsystem.com/components/text-input/usage/ | Confirmed 48 px as the large simple-form input height and the principle that field width should be proportional to expected content. Affects only login CSS and does not alter authentication or engineering behavior. Accessed 2026-07-15. |
| Login input content fit | U.S. Web Design System text input, https://designsystem.digital.gov/components/text-input/ | Confirmed that field length should hint at expected response length and that explicit width variants are preferable to unrestricted full-width desktop fields. Affects only login CSS. Accessed 2026-07-15. |
