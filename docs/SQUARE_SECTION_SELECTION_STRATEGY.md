...
当前策略是：用历史标准命令流生成同一类 S2 模型，用相似提资学习和截面几何量确定第一个候选，用 ANSYS 真实试算确认应力比；若第一次不在经济区间，再按截面模量做一次修正试算。最后仍以 `result_validation.json` 和 `evaluation_summary.json` 的确定性比值作为门禁。
...
2. 候选必须先被提资“计算说明”允许列表过滤；未列出的截面不得试算。
3. 新提资没有方钢截面时，相似提资缓存或确定性工程估算只用于确定第一个候选，不直接复用历史结果。
4. 生产经济区间为 `0.60 <= ratio <= 0.9999`。落入该区间即认为经济性满足并停止。
5. 正常最多运行两次真实 ANSYS 候选试算：第一次为学习/估算候选；若第一次 `ratio > 0.9999` 或 `< 0.60`，按“应力比与截面模量近似反比”的趋势做一次截面模量修正候选。
6. 候选试算阶段不因缺最终报告图片继续扫截面；正式输出阶段仍检查图片完整性。
7. 发现连续多次都是焊缝或螺栓控制，而方钢立柱/方钢构件已经满足时，停止继续放大方钢，改为连接/焊缝/提取集合问题，不硬凑通过。
8. 选出的截面必须满足完整确定性比值 `<= 1.0`；`> 1.0` 必须失败。若允许截面内无法在两次内得到满足截面，系统应给出可审查的失败原因或允许截面不足结论，不得扩大到未允许截面。
...
1. 一个相似学习或工程估算候选；
2. 如果第一次不在 `0.60-0.9999` 经济区间，再跑一个按截面模量修正后的候选；
3. 如果控制项不是方钢、输出源缺失、全零、UNKNOWN、超时或输出停滞，提前退出并修数据/命令流，不继续放大截面。
