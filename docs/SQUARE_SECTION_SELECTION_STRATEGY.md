...
当前策略是：用历史标准命令流生成同一类 S2 模型，用相似提资、载荷路径、反应谱强度和截面几何量确定第一个候选，用 ANSYS 真实试算确认全部评定比值；若存在预计可满足且材料成本明显更低的允许截面，再做一次低成本候选试算。最后仍以 `result_validation.json` 和 `evaluation_summary.json` 的确定性比值作为合格门禁。
...
4. `0.60 <= ratio <= 0.9999` 只作为利用率复核提示，不是合格门禁，也不单独证明经济性。若最轻、最低材料成本的允许截面已经通过，即使比值低于 0.60，也不应仅为追求高比值而换截面。
5. 正常最多运行两次真实 ANSYS 候选试算：第一次为学习/估算候选；第一次超限时按控制构件和截面模量估算更强候选，第一次通过时只在存在材料成本至少降低 2% 且具有通过可能的允许截面时增加一次低成本试算。
...
10. 当前成本只覆盖方钢材料：按理论质量和带日期、来源的市场参考吨价排序；托臂、焊缝、加工、涂层、运输及合同采购价格未配置，不得宣称全寿期或总造价最优。
...
1. 一个相似学习或工程估算候选；
2. 第一次超限时跑一个按控制构件/截面模量修正后的候选；第一次通过时，仅在存在明显更低材料成本且具有通过可能的候选时补跑一次；
