...
当前策略是：用历史标准命令流生成同一类 S2 模型，用相似提资、载荷路径、反应谱强度和截面几何量确定第一个候选，用 ANSYS 真实试算确认全部评定比值；若存在预计可满足且方钢截面材料用量明显更低的允许截面，再做一次候选试算。最后仍以 `result_validation.json` 和 `evaluation_summary.json` 的确定性比值作为合格门禁。
...
4. `0.60 <= ratio <= 0.9999` 只作为利用率复核提示，不是合格门禁，也不单独证明经济性。若材料面积/估算理论质量最低的允许截面已经通过，即使比值低于 0.60，也不应仅为追求高比值而换截面。
5. 正常最多运行两次真实 ANSYS 候选试算：第一次为学习/估算候选；第一次超限时按控制构件和截面模量估算更强候选，第一次通过时只在存在材料用量至少降低 2% 且具有通过可能的允许截面时增加一次试算。
6. 候选试算阶段不因缺最终报告图片继续扫截面；正式输出阶段仍检查图片完整性。
7. 发现连续多次都是焊缝或螺栓控制，而方钢立柱/方钢构件已经满足时，停止继续放大方钢，改为连接/焊缝/提取集合问题，不硬凑通过。
8. 选出的截面必须满足全部有效确定性比值 `<= 1.0`；`> 1.0` 必须失败。正常两次试算均超限时可继续验证更大的提资允许截面，但不得扩大到未允许截面。
9. 支架间距和方钢长度由上游布置固定，自动选型不得修改。允许最大截面仍超限时，平台阻断并允许操作人员在计算工作台明确修改线载荷后重新计算，不自动降低载荷。
10. 默认经济性指标只覆盖主方钢截面的材料面积、估算理论质量和几何外表面积，不输出人民币结论，也不得宣称核电工程综合造价最优。只有导入单位核定、有效期内、完整截面精确匹配的价格表后，才允许显示带表号和版次的材料参考；托臂、托盘、螺栓、焊缝、加工、检验、防腐、运输和安装仍需另行计价。
...
1. 一个相似学习或工程估算候选；
2. 第一次超限时跑一个按控制构件/截面模量修正后的候选；第一次通过时，仅在存在明显更低方钢材料用量且具有通过可能的候选时补跑一次；
