...
def _item(check_id: str, category: str, calculation: float, allowable: float, source_ref: str) -> dict:
...
def bolt_stresses_from_group_loads(
    ...
    *,
    ...
    lever_arm_m: float = BOLT_GROUP_LEVER_ARM_M,
) -> dict[str, float]:
    """Replicate the confirmed workbook path from bolt group loads to stresses.

    Formula source: `螺栓!E51:E57`.
    E51 = sqrt(FX^2 + FY^2) / 2 + MZ / L
    E52 = FZ / 2 + MY / L
    E53 = E51 / S / 1e6 / 0.9
    E54 = E52 / S / 1e6
    """
    shear_force_n = (fx_n**2 + fy_n**2) ** 0.5 / 2.0 + abs(mz_nm) / lever_arm_m
    tension_force_n = abs(fz_n) / 2.0 + abs(my_nm) / lever_arm_m
...
def _normalise_bolt_result(row: dict) -> dict | None:
    ...
    enriched_values.update(
        {
            "tension_mpa": {
                ...
                "unit": "MPa",
                "source_ref": SOURCE_BOLT_FORCE_TO_STRESS,
            },
            "shear_mpa": {
    ...
    enriched["bolt_stress_source_ref"] = SOURCE_BOLT_FORCE_TO_STRESS
    return enriched


def evaluate_bolt_forces(bolt_results: list[dict]) -> list[dict]:
    items: list[dict] = []
    for row in bolt_results:
        row = _normalise_bolt_result(row)
        if row is None:
            continue
        name = row["name"]
        ...
        items.append(_item(f"{prefix}_bolt_tension", "机械螺栓拉伸", tension, allowable_tension, SOURCE_BOLT_DIRECT))
        items.append(_item(f"{prefix}_bolt_shear", "机械螺栓剪切", shear, allowable_shear, SOURCE_BOLT_DIRECT))
        items.append(
            {
                ...
                "unit": None,
                "source_ref": SOURCE_BOLT_COMBINED if combined is not None else TODO_SOURCE,
                "formula_status": formula_status(SOURCE_BOLT_COMBINED if combined is not None else TODO_SOURCE),
                "notes": "Excel formula pattern: tension_ratio^2 + shear_ratio^2." if combined is not None else "allowable value is missing or zero",
            }
        )
    return items
